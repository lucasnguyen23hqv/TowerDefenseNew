...
public class TankerEnemy extends Rectangle implements Enemy {
    private int xE, yE;
    private int shield = 42*3/4;
    private int health = Screen.room.blockSize*3;
    ...
    private int walkFrame = 0, walkSpeed = 40;
    ...
    public void spawnEnemy(){
        for(int i = 0; i< Screen.room.block.length; i++){
            if(Screen.room.block[i][0].groundID == Value.groundRoad){
                setBounds(Screen.room.block[i][0].x-17, Screen.room.block[i][0].y-16, enemysize+20, enemysize+20);
                xE = 0;
                yE = i;
            }
        }
    ...
    public void draw(Graphics g) {
        if (ingame) {
            ...
            g.fillRect(x, y- (healthSpace+healthHeight), save_health/3, healthHeight+1);
            g.setColor(new Color(50, 180, 50));
            g.fillRect(x, y - (healthSpace + healthHeight), health/3, healthHeight);
            g.setColor(new Color(0xABA1A1));
            g.fillRect(x + (enemysize-shield), y- (healthSpace+healthHeight), shield, healthHeight);
            g.setColor(new Color(0, 0, 0));
            g.drawRect(x, y - healthSpace - healthHeight, health/3-1, healthHeight);
        }
    }
